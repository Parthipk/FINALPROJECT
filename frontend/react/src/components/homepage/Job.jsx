...
import { FaBriefcase, FaMapMarkerAlt, FaSuitcase, FaMailBulk } from 'react-icons/fa'; // Added FaMailBulk import
...
export default function Job() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        console.log('Fetched Jobs:', response.data);  // Log the fetched jobs to verify
        if (Array.isArray(response.data) && response.data.length > 0) {
          setJobs(response.data);
          filterJobs(response.data); // Apply filtering
        } else {
    ...
    // Interval to periodically refresh the job list
  ...
  // Filter jobs based on saved cookies (search criteria)
  const filterJobs = (jobs) => {
    const savedJob = Cookies.get('job');
    const savedLocation = Cookies.get('location');
    const savedCategory = Cookies.get('category');

    console.log('Filtering jobs with criteria:', { savedJob, savedLocation, savedCategory });

    const filtered = jobs.filter((job) => {
      const matchesJob = savedJob ? job.jobTitle.toLowerCase().includes(savedJob.toLowerCase()) : true;
      const matchesLocation = savedLocation ? job.companyLocation.toLowerCase().includes(savedLocation.toLowerCase()) : true;
      const matchesCategory = savedCategory ? job.workModel.toLowerCase().includes(savedCategory.toLowerCase()) : true;
  ...
  // Open the modal and set selected job
  const openModal = (job) => {
    console.log('Opening Modal for Job:', job); // Log when modal is opened
    setSelectedJob(job);
    setModalOpen(true);
  };
  ...
  // Close the modal
  const closeModal = () => {
    console.log('Closing Modal'); // Log when modal is closed
    setModalOpen(false);
    setSelectedJob(null);
  };
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50 mt-16">
          <div className="bg-white p-6 max-[80%] w-4/5 relative rounded-lg shadow-xl space-y-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex flex-col items-center">
              ...
              <img
                src={image} // Display job image or the default image
                alt={selectedJob.jobTitle}
                className="w-[50%] h-[30%] object-cover rounded-lg mb-6"
              />
              ...
              <div className="flex justify-between gap-4 w-full mt-6">
                <button
                  onClick={() => window.location.href = `mailto:${selectedJob.email}`} // Open email client when clicked
                  className="px-6 py-2 bg-[#309689] text-white rounded-md"
                >
                  Apply Now
                </button>
              </div>
